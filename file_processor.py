...
def process_case_upload(request, app):
    ...
    try:
        ...
        # 验证字段关系 - 接收所有返回值
        # 核心修改：在解包时添加 confiscation_of_property_amount_indices
        (mismatch_indices, gender_mismatch_indices, age_mismatch_indices, brief_case_details_mismatch_indices, issues_list, 
         ...
         confiscation_of_property_amount_indices) = validate_case_relationships(df) # <--- 【关键】这里添加 confiscation_of_property_amount_indices


        # 生成副本和立案编号文件 - 传递所有参数
        # 核心修改：在调用 generate_case_files 时添加 confiscation_of_property_amount_indices
        copy_path, case_num_path = generate_case_files(
            ...
            trial_report_detention_mismatch_indices,
            confiscation_amount_indices,
            confiscation_of_property_amount_indices # <--- 【关键】这里添加 confiscation_of_property_amount_indices
        )
