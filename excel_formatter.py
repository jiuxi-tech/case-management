...
# 导入 get_validation_issues 和 extract_name_from_report 但它们在此文件中未直接调用，仅为上下文保留
...
def format_excel(df, mismatch_indices, output_path, issues_list):
    ...
    issues_list: 从 validation_core.py 中的 get_validation_issues 函数获取的问题列表，
    ...
    with pd.ExcelWriter(output_path, engine='xlsxwriter', engine_kwargs={'options': {'nan_inf_to_errors': True}}) as writer:
        ...
        for idx in range(len(df)):
            ...
            if Config.COLUMN_MAPPINGS["acceptance_time"] in df.columns:
                ...
                # 关键修改：issues_list 中的元素现在有3个，所以解包时需要匹配
            ...
            # 处置情况报告为空
            report_text = row.get("处置情况报告", '') # 使用.get避免KeyError，如果列不存在
            # 关键修改：issues_list 中的元素现在有3个，所以解包时需要匹配
            if pd.isna(report_text) or report_text == '': # 确保处理了空值和空字符串
