...
def validate_disposal_and_amount_rules(df, issues_list, disposal_spirit_mismatch_indices, closing_time_mismatch_indices):
    """
    验证与处分和金额相关的规则。
    ...
    try:
        ...
        col_closing_time = "结案时间" # 新增列名

        required_cols = [col_spirit_violation, col_disposal_decision, col_closing_time] # 更新所需列
    ...
    for index, row in df.iterrows():
        ...
        excel_closing_time = None
        if pd.notna(row[col_closing_time]):
            try:
                # 尝试将Excel中的结案时间转换为日期对象，忽略时间部分
                if isinstance(row[col_closing_time], datetime):
                    excel_closing_time = row[col_closing_time].date()
                else:
                    # pd.to_datetime 可以很好地处理多种日期格式
                    excel_closing_time = pd.to_datetime(row[col_closing_time]).date()
            except Exception as e:
                logger.warning(f"行 {index + 1} - 无法解析 '结案时间' 字段 '{row[col_closing_time]}': {e}")
                print(f"行 {index + 1} - 无法解析 '结案时间' 字段 '{row[col_closing_time]}': {e}")
        ...
        if match:
            ...
            try:
                ...
                print(f"行 {index + 1} - 从处分决定中提取的生效日期: '{date_str}' (格式化后: {extracted_disposal_date})")
            except ValueError as ve:
                logger.warning(f"行 {index + 1} - 无法解析处分决定中提取的日期 '{date_str}': {ve}")
                print(f"行 {index + 1} - 无法解析处分决定中提取的日期 '{date_str}': {ve}")
        else:
            logger.info(f"行 {index + 1} - 处分决定中未找到匹配的生效日期模式。")
            print(f"行 {index + 1} - 处分决定中未找到匹配的生效日期模式。")

        # 进行“结案时间”与提取日期之间的比对
        if excel_closing_time and extracted_disposal_date:
            if excel_closing_time != extracted_disposal_date:
                issues_list.append((index, case_code, person_code, "BN结案时间与CU处分决定不一致"))
                closing_time_mismatch_indices.add(index)
                logger.warning(f"行 {index + 1} - 规则违规: '结案时间' ('{excel_closing_time}') 与处分决定中提取的生效日期 ('{extracted_disposal_date}') 不一致。")
                print(f"行 {index + 1} - 规则违规: '结案时间' ('{excel_closing_time}') 与处分决定中提取的生效日期 ('{extracted_disposal_date}') 不一致。")
            else:
                logger.info(f"行 {index + 1} - '结案时间' 字段值与处分决定中提取的生效日期一致。")
                print(f"行 {index + 1} - '结案时间' 字段值与处分决定中提取的生效日期一致。")
        elif excel_closing_time is None:
            logger.info(f"行 {index + 1} - '结案时间' 字段为空，跳过比对。")
            print(f"行 {index + 1} - '结案时间' 字段为空，跳过比对。")
        elif extracted_disposal_date is None:
            logger.info(f"行 {index + 1} - 未能从处分决定中提取到生效日期，跳过比对。")
            print(f"行 {index + 1} - 未能从处分决定中提取到生效日期，跳过比对。")
