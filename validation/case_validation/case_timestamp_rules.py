...
def validate_filing_time(df, issues_list, filing_time_mismatch_indices, disciplinary_committee_filing_time_mismatch_indices, supervisory_committee_filing_time_mismatch_indices, disciplinary_committee_filing_authority_mismatch_indices, supervisory_committee_filing_authority_mismatch_indices, app_config):
    """
    验证“立案时间”、“纪委立案时间”和“监委立案时间”字段的规则。
    ...
    # 获取列名，确保存在
    try:
        col_filing_time = app_config['COLUMN_MAPPINGS']['filing_time']
        col_filing_decision_doc = app_config['COLUMN_MAPPINGS']['filing_decision_doc']
        col_disciplinary_committee_filing_time = app_config['COLUMN_MAPPINGS']['disciplinary_committee_filing_time']
        col_disciplinary_committee_filing_authority = app_config['COLUMN_MAPPINGS']['disciplinary_committee_filing_authority']
        col_supervisory_committee_filing_time = app_config['COLUMN_MAPPINGS']['supervisory_committee_filing_time']
        col_supervisory_committee_filing_authority = app_config['COLUMN_MAPPINGS']['supervisory_committee_filing_authority']
        col_reporting_unit_name = app_config['COLUMN_MAPPINGS']['reporting_agency']
    ...
    for index, row in df.iterrows():
        excel_filing_time_raw = str(row.get(col_filing_time, "")).strip() if pd.notna(row.get(col_filing_time)) else ''
        filing_decision_doc_text = str(row.get(col_filing_decision_doc, "")).strip() if pd.notna(row.get(col_filing_decision_doc)) else ''
        excel_disciplinary_committee_filing_time_raw = str(row.get(col_disciplinary_committee_filing_time, "")).strip() if pd.notna(row.get(col_disciplinary_committee_filing_time)) else ''
        excel_disciplinary_committee_filing_authority = str(row.get(col_disciplinary_committee_filing_authority, "")).strip() if pd.notna(row.get(col_disciplinary_committee_filing_authority)) else ''
        excel_supervisory_committee_filing_time_raw = str(row.get(col_supervisory_committee_filing_time, "")).strip() if pd.notna(row.get(col_supervisory_committee_filing_time)) else ''
        excel_supervisory_committee_filing_authority = str(row.get(col_supervisory_committee_filing_authority, "")).strip() if pd.notna(row.get(col_supervisory_committee_filing_authority)) else ''
        excel_reporting_unit_name = str(row.get(col_reporting_unit_name, "")).strip() if pd.notna(row.get(col_reporting_unit_name)) else ''
        ...
        if original_filing_decision_timestamp_str and \
           (re.search(r'\s', original_filing_decision_timestamp_str) or '\xa0' in original_filing_decision_timestamp_str):
            issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("filing_decision_timestamp_space", "立案决定书落款时间存在空格"), "中")) # 增加风险等级
            filing_time_mismatch_indices.add(index)
            logger.info(f"行 {index + 1} - 规则1违规: 立案决定书落款时间存在空格，原始时间为: '{original_filing_decision_timestamp_str}'")
        ...
        if standardized_excel_filing_time and standardized_filing_decision_timestamp:
            if standardized_excel_filing_time != standardized_filing_decision_timestamp:
                issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_filing_time_with_decision", "立案时间与立案决定书落款时间不一致"), "高")) # 增加风险等级
                filing_time_mismatch_indices.add(index)
                logger.info(f"行 {index + 1} - 规则2违规: Excel立案时间标准化后 ('{standardized_excel_filing_time}') 与决定书时间标准化后 ('{standardized_filing_decision_timestamp}') 不一致。")
            else:
                logger.info(f"行 {index + 1} - 立案时间 ('{standardized_excel_filing_time}') 与决定书时间 ('{standardized_filing_decision_timestamp}') 一致。")
            ...
            issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_filing_time_with_decision_missing", "立案时间与立案决定书落款时间不一致（一方缺失或格式问题）"), "高")) # 增加风险等级
            filing_time_mismatch_indices.add(index)
            logger.warning(f"行 {index + 1} - 规则2违规: Excel立案时间标准化后 ('{standardized_excel_filing_time}') 或决定书时间标准化后 ('{standardized_filing_decision_timestamp}') 缺失/格式错误，导致不一致。")
        ...
        if filing_decision_doc_text and \
           ("纪立" in filing_decision_doc_text and ("审查" in filing_decision_doc_text or "审查调查" in filing_decision_doc_text)):
            ...
            if standardized_excel_disciplinary_committee_filing_time and standardized_filing_decision_timestamp:
                if standardized_excel_disciplinary_committee_filing_time != standardized_filing_decision_timestamp:
                    issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_disciplinary_filing_time", "纪委立案时间和立案决定书落款时间不一致"), "高")) # 增加风险等级
                    disciplinary_committee_filing_time_mismatch_indices.add(index)
                    logger.info(f"行 {index + 1} - 规则3a违规: Excel纪委立案时间标准化后 ('{standardized_excel_disciplinary_committee_filing_time}') 与决定书时间标准化后 ('{standardized_filing_decision_timestamp}') 不一致。")
                else:
                    logger.info(f"行 {index + 1} - 规则3a: 纪委立案时间 ('{standardized_excel_disciplinary_committee_filing_time}') 与决定书时间 ('{standardized_filing_decision_timestamp}') 一致。")
                ...
                issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_disciplinary_filing_time_missing", "纪委立案时间和立案决定书落款时间不一致（一方缺失或格式问题）"), "高")) # 增加风险等级
                disciplinary_committee_filing_time_mismatch_indices.add(index)
                logger.warning(f"行 {index + 1} - 规则3a违规: Excel纪委立案时间标准化后 ('{standardized_excel_disciplinary_committee_filing_time}') 或决定书时间标准化后 ('{standardized_filing_decision_timestamp}') 缺失/格式错误，导致不一致。")
            ...
            if not found_match_disciplinary:
                issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_disciplinary_authority", "填报单位名称与纪委立案机关不一致"), "高")) # 增加风险等级
                disciplinary_committee_filing_authority_mismatch_indices.add(index)
                logger.info(f"行 {index + 1} - 规则3b违规: 纪委立案机关 ('{excel_disciplinary_committee_filing_authority}') 和填报单位名称 ('{excel_reporting_unit_name}') 在对应表中未找到匹配记录，或category不是'NSL'。")
            else:
                logger.info(f"行 {index + 1} - 规则3b: 纪委立案机关 ('{excel_disciplinary_committee_filing_authority}') 和填报单位名称 ('{excel_reporting_unit_name}') 在对应表中找到匹配记录。")
        ...
        if filing_decision_doc_text and \
           ("监立" in filing_decision_doc_text and "调查" in filing_decision_doc_text):
            ...
            if standardized_excel_supervisory_committee_filing_time and standardized_filing_decision_timestamp:
                if standardized_excel_supervisory_committee_filing_time != standardized_filing_decision_timestamp:
                    issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_supervisory_filing_time", "监委立案时间和立案决定书落款时间不一致"), "高")) # 增加风险等级
                    supervisory_committee_filing_time_mismatch_indices.add(index)
                    logger.info(f"行 {index + 1} - 规则4a违规: Excel监委立案时间标准化后 ('{standardized_excel_supervisory_committee_filing_time}') 与决定书时间标准化后 ('{standardized_filing_decision_timestamp}') 不一致。")
                else:
                    logger.info(f"行 {index + 1} - 规则4a: 监委立案时间 ('{standardized_excel_supervisory_committee_filing_time}') 与决定书时间 ('{standardized_filing_decision_timestamp}') 一致。")
                ...
                issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_supervisory_filing_time_missing", "监委立案时间和立案决定书落款时间不一致（一方缺失或格式问题）"), "高")) # 增加风险等级
                supervisory_committee_filing_time_mismatch_indices.add(index)
                logger.warning(f"行 {index + 1} - 规则4a违规: Excel监委立案时间标准化后 ('{standardized_excel_supervisory_committee_filing_time}') 或决定书时间标准化后 ('{standardized_filing_decision_timestamp}') 缺失/格式错误，导致不一致。")
            ...
            if not found_match_supervisory:
                issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_supervisory_authority", "填报单位名称与监委立案机关不一致"), "高")) # 增加风险等级
                supervisory_committee_filing_authority_mismatch_indices.add(index)
                logger.info(f"行 {index + 1} - 规则4b违规: 监委立案机关 ('{excel_supervisory_committee_filing_authority}') 和填报单位名称 ('{excel_reporting_unit_name}') 在对应表中未找到匹配记录，或category不是'NSL'。")
            else:
                logger.info(f"行 {index + 1} - 规则4b: 监委立案机关 ('{excel_supervisory_committee_filing_authority}') 和填报单位名称 ('{excel_reporting_unit_name}') 在对应表中找到匹配记录。")
           ...
           ("纪监立" in filing_decision_doc_text and "审查调查" in filing_decision_doc_text):
            ...
            if standardized_filing_decision_timestamp:
                if (standardized_excel_disciplinary_committee_filing_time and standardized_excel_disciplinary_committee_filing_time != standardized_filing_decision_timestamp) or \
                   (standardized_excel_supervisory_committee_filing_time and standardized_excel_supervisory_committee_filing_time != standardized_filing_decision_timestamp):
                    mismatch_found_rule_5a = True
                    issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("inconsistent_dual_filing_time", "监委立案时间、纪委立案时间和立案决定书落款时间不一致"), "高")) # 增加风险等级
                    disciplinary_committee_filing_time_mismatch_indices.add(index) # 标红纪委立案时间
                    supervisory_committee_filing_time_mismatch_indices.add(index) # 标红监委立案时间
                    logger.info(f"行 {index + 1} - 规则5a违规: 监委/纪委立案时间与决定书落款时间不一致。决定书时间: '{standardized_filing_decision_timestamp}', 纪委时间: '{standardized_excel_disciplinary_committee_filing_time}', 监委时间: '{standardized_excel_supervisory_committee_filing_time}'")
                else:
...
def validate_confiscation_amount(df, issues_list, confiscation_amount_indices, app_config):
    """
    新增规则：与“审理报告”字段内容进行对比，查找字符串“收缴”，
    若出现“收缴”二字，将副本文件“收缴金额（万元）”字段标黄，
    并在立案编号表中添加问题描述：“CY审理报告中含有收缴二字，请人工再次确认“。
    ...
    col_confiscation_amount = app_config['COLUMN_MAPPINGS']['confiscation_amount']
    col_case_code = app_config['COLUMN_MAPPINGS']['case_code']
    col_person_code = app_config['COLUMN_MAPPINGS']['person_code']
    ...
    for index, row in df.iterrows():
        ...
        if "收缴" in trial_report_text:
            issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("highlight_collection_amount", "审理报告中含有收缴二字，请人工再次确认"), "中")) # 增加风险等级
            confiscation_amount_indices.add(index)
            logger.info(f"行 {index + 1} - '{col_trial_report}' 中包含 '收缴'。'{col_confiscation_amount}' 字段将标黄。案件编码: {case_code}, 涉案人员编码: {person_code}")

    logger.info("收缴金额相关规则验证完成。")


def validate_confiscation_of_property_amount(df, issues_list, confiscation_of_property_amount_indices, app_config):
    """
    ...
    col_trial_report = app_config['COLUMN_MAPPINGS']['trial_report']
    col_confiscation_of_property_amount = app_config['COLUMN_MAPPINGS']['confiscation_of_property_amount']
    col_case_code = app_config['COLUMN_MAPPINGS']['case_code']
    col_person_code = app_config['COLUMN_MAPPINGS']['person_code']
    ...
    for index, row in df.iterrows():
        ...
        if "没收金额" in trial_report_text:
            issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("highlight_confiscation_of_property_amount", "审理报告中含有没收金额四字，请人工再次确认没收金额"), "中")) # 增加风险等级
            confiscation_of_property_amount_indices.add(index)
            logger.info(f"行 {index + 1} - '{col_trial_report}' 中包含 '没收金额'。'{col_confiscation_of_property_amount}' 字段将标黄。案件编码: {case_code}, 涉案人员编码: {person_code}")
...
def validate_registered_handover_amount(df, issues_list, registered_handover_amount_indices, app_config):
    ...
    for index, row in df.iterrows():
        ...
        if "登记上交金额" in trial_report_text:
            issues_list.append((index, case_code, person_code, app_config['VALIDATION_RULES'].get("highlight_registration_amount", "审理报告中含有登记上交金额，请人工再次确认登记上交金额"), "中")) # 增加风险等级
            registered_handover_amount_indices.add(index)
            logger.info(f"行 {index + 1} - '{col_trial_report}' 中包含 '登记上交金额'。'{col_registered_handover_amount}' 字段将标黄。案件编码: {case_code}, 涉案人员编码: {person_code}")
